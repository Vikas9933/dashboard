...
-- Run this ONCE in Supabase SQL Editor, then create user in Authentication UI
-- =============================================================================

-- 1. Remove strict rules that block new user signup
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_role_agency_check;
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_agent_team_check;
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_team_leader_team_check;

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_role_agency_check CHECK (
    NOT is_active OR role IN ('admin', 'super_admin') OR agency_id IS NOT NULL
  );

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_agent_team_check CHECK (
    NOT is_active OR role <> 'agent' OR team_id IS NOT NULL
  );

ALTER TABLE public.profiles
  ADD CONSTRAINT profiles_team_leader_team_check CHECK (
    NOT is_active OR role <> 'team_leader' OR team_id IS NOT NULL
  );

-- 2. RLS blocks the signup trigger — disable on profiles until policies are added
ALTER TABLE public.profiles DISABLE ROW LEVEL SECURITY;

-- 3. Fix trigger: first user is admin; others stay inactive until assigned
